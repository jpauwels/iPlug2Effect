...
if %DEMO_ARG% == "demo" (
  echo Making SimplestEffect Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making SimplestEffect Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild SimplestEffect-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild SimplestEffect.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild SimplestEffect.sln /t:SimplestEffect-app;SimplestEffect-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SimplestEffect.aaxplugin\Contents\Win32\SimplestEffect.aaxplugin --out .\build-win\aax\bin\SimplestEffect.aaxplugin\Contents\Win32\SimplestEffect.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SimplestEffect.aaxplugin\Contents\x64\SimplestEffect.aaxplugin --out .\build-win\aax\bin\SimplestEffect.aaxplugin\Contents\x64\SimplestEffect.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\SimplestEffect.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\SimplestEffect.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "SimplestEffect Installer" ".\installer\SimplestEffect Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\SimplestEffect Installer.exe" ".\installer\SimplestEffect Demo Installer.exe"
  REM -del ".\installer\SimplestEffect Installer.exe"
  REM -)
