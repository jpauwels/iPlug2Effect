...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\SimplestEffect Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\SimplestEffect Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\SimplestEffect manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\SimplestEffect.vst3\\Contents\\x86_64-win\\SimplestEffect.vst3",
      projectpath + "\\build-win\\SimplestEffect_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\SimplestEffect-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SimplestEffect-app_x64.pdb"  
  ]
